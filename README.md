...
但本次验证中发现，当前环境命中的 `.venv/Scripts/mmdc.exe` 实际是 Python 版 `mmdc`（PhantomJS）。

- 它可能在 `stderr` 已报错时仍返回成功状态
- 它可能生成 400x305 的空白 PNG，造成“图像渲染成功”的假象
- 对 `classDiagram`、中文标签和较新的 Mermaid 语法兼容性较差

因此代码已经新增空白 PNG 和隐藏错误检测；若需要继续稳定渲染 Mermaid 图，建议改用 Node 版 `@mermaid-js/mermaid-cli`，并通过 `--cli-command` 显式指定，例如 `npx.cmd -y @mermaid-js/mermaid-cli` 或已安装的 `mmdc.cmd`

### 5. 纯文本页的自动续页仍属于保守策略

当前仅对“纯文本页面、只有一个主正文框、模板说明明确写有可分多页”的场景启用自动续页。

这样可以降低误拆分页的概率；同时本轮已补充对表格页和图文混排页的保守续页能力，用于避免表格与下方元素重叠。

本次又进一步做了两项修正：

- 正文容量估算会结合模板里的字号与行距提示，拆页判断更保守
- 复制续页时会移除 placeholder 元数据，避免标题框依赖母版继承后出现位置漂移
- 表格规则现在会保存模板里的字体、字号、行高、列宽、对齐和边距信息；若模板缺失样式，则回退到“正文 微软雅黑 14 磅”
- PPT 组装阶段会根据表格真实行数和估算行高重新计算占位，必要时把表格及其后续元素挪到续页，降低越界和重叠概率

按当前验证结果，第 16 页的超长正文已从 2 页调整为 3 页，续页标题框坐标保持一致。
...
- 架构图 Mermaid 策略已从 `architecture-beta` 切换为 `classDiagram`
- 新增 Mermaid 归一化逻辑，可把旧的 `architecture-beta` 输出转换为 `classDiagram`
- 已修复 Python 版 `mmdc` 的“空白图仍判成功”问题，当前会明确报错并提示切换 Node 版 CLI
- 纯文本续页的标题位置和第一页超框问题已做保守修正
- 模板解析已开始提取表格样式，`template_rules.json` 中的表格节点会带上字体、字号、行高、列宽及表头/正文样式
- 表格写回时会按模板样式重新设置单元格字体，并按真实内容估算行高
- 混合页面会按文本、表格、图片的垂直占位做保守续页，尽量避免表格越界和与下方元素重叠
