...
from pptx_gen.schemas import BoundingBox, ElementRule, PageRule, SlideSize, TableSchema, TemplateMeta, TemplateRules, TitleRule
...
class TemplateRuleParser:
    ...
    def save(self, output_path: str | Path) -> TemplateRules:
        ...
        rules = self.parse()
        output.write_text(json.dumps(rules.to_dict(), ensure_ascii=False, indent=2), encoding="utf-8")
        return rules
    ...
    def _parse_slide(self, archive: zipfile.ZipFile, slide_name: str, slide, page_no: int) -> PageRule:
        ...
        for child in list(sp_tree):
            ...
            if tag == "sp":
                ...
                parsed = self._parse_picture(child, page_no, z_order)
                z_order += 1
                if parsed is not None:
                    elements.append(parsed)
            else:
        ...
        supports_mermaid = any(item.type == "image" for item in elements)
        ...
        return PageRule(
            ...
            supports_mermaid=supports_mermaid,
            title=title_info,
            elements=elements,
        )
    ...
    def _parse_picture(self, node: ET.Element, page_no: int, z_order: int) -> ElementRule | None:
        c_nv_pr = node.find("./p:nvPicPr/p:cNvPr", self.NS)
        xfrm = node.find("./p:spPr/a:xfrm", self.NS)
        if c_nv_pr is None:
            return None
        shape_id = int(c_nv_pr.attrib.get("id", "0"))
    ...
    def _infer_page_purpose(self, page_name: str, elements: list[ElementRule]) -> str:
        ...
        if any(item.type == "table" for item in elements):
            return "table"
        if any(item.type == "image" for item in elements):
            return "diagram"
        return "text"
    ...
    def _infer_diagram_kind(self, shape_name: str) -> str:
        lowered = shape_name.lower()
        if "时序" in shape_name or "sequence" in lowered:
            return "sequence"
        if "流程" in shape_name or "flow" in lowered:
            return "flowchart"
        return "architecture"
